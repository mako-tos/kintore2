...
const Harness: React.FC<{ onRef: (api: any) => void }> = ({ onRef }) => {
  ...
  return (
    <div>
      ...
      <div aria-label="set">{api.set}</div>
...
describe("useTrainingRecordForm", () => {
  test("start increments set and toggles running", () => {
  ...
  test("menu change triggers autosave; failure prevents switch", async () => {
    ...
    expect(screen.getByLabelText("set").textContent).toBe("1");
    await act(async () => {
      fireEvent.click(screen.getByText("switchM2"));
    });
