...
async function request<T = any>(
  ...
  if (!res.ok) {
    throw new ApiError({
      status: res.status,
      message: maybeBody?.message || res.statusText,
      details: maybeBody,
    });
  }
...
export const apiClient = {
  ...
  post: <T>(url: string, json?: any, opts?: RequestOptions) =>
    request<T>(url, { ...opts, method: "POST", json }),
  put: <T>(url: string, json?: any, opts?: RequestOptions) =>
    request<T>(url, { ...opts, method: "PUT", json }),
  patch: <T>(url: string, json?: any, opts?: RequestOptions) =>
